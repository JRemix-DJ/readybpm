...
class Generos extends CI_Controller {
    ...
    public function genero(){
        ...
        if($this->is_gender($gender_id)){
            ...
            // Primero, verificamos el caso especial del género 45 (Drops)
            if ($gender_id == 45) {
                $where['product_type_id'] = 1; // Drops siempre son audios
            } else {
                // Para todos los demás géneros, respetamos la sección actual del usuario
                if ($this->session->userdata('content_type') == 'videos') {
                    $where['product_type_id'] = 3; // Si está en la sección de videos
                } else {
                    $where['product_type_id'] = 1; // Si está en la sección de audios
                }
            }
