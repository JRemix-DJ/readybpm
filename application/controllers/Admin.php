...
class Admin extends CI_Controller
{
        ...
        if ( $this->user_has_admin_access() ) {
            ...
            if ( $this->session->userdata('is_logued_in') ) {
			 ...
			 <link href="'.base_url().'admin_assets/lib/select2/css/select2.min.css" rel="stylesheet">';
                ...
                // Lógica para determinar si se buscan aprobados o no aprobados
                if ( isset($_GET['aprobacion']) ) {
                    $where['approved'] = 0; // Productos sin aprobar
                } else {
                    $where['approved'] = 1; // Productos aprobados
                }

                // Lógica para filtrar por editor si no es admin
                if ( $user_role != 'is_admin' && $user_role != "is_subadmin" ) {
                ...
                $this->load->view('admin/side');
                $this->load->view('admin/top');
                $this->load->view('admin/productos' , $data); // Pasamos $data a la vista
                $this->load->view('admin/footer');
            } else {
                redirect(base_url().'admin/login/');
            }
    ...
    public function subir()
    {
        ...
        if ( $action == 'process_main_file' ) {
            ...
            if ( move_uploaded_file($tmp_path , $descargable_path) ) {
                ...
                if ( $this->input->post('generate_demo') === 'true' ) {
                    $demo_path = $this->path_preview.$unique_filename;
                    $this->create_mp3_preview($descargable_path , $demo_path);
                }
                echo json_encode(['success' => true , 'filename' => $unique_filename]);
            } else {
                echo json_encode(['success' => false , 'error' => 'No se pudo mover el archivo principal.']);
            }

        } elseif ( $action == 'process_demo_file' ) {
            if ( !isset($_FILES['files']) || $_FILES['files']['error'] !== UPLOAD_ERR_OK ) {
                echo json_encode(['success' => false , 'error' => 'Error en la subida del demo.']);
                return;
            }
            $tmp_path = $_FILES['files']['tmp_name'];
            $unique_filename = $user_id.time().'_demo.'.$extension;
            $demo_path = $this->path_preview.$unique_filename;

            if ( move_uploaded_file($tmp_path , $demo_path) ) {
                echo json_encode(['success' => true , 'filename' => $unique_filename]);
            } else {
                echo json_encode(['success' => false , 'error' => 'No se pudo mover el demo manual.']);
            }
    ...
    function audio($action)
    {
        ...
        $productos_data = array(
            ...
            'artist' => $this->input->post('video_artist') ,
            'price' => $this->input->post('price') ,
            ...
            'descargable' => $this->input->post('descargable') ,
            'format' => 'audio' ,
            'duration' => $duration
        );

        // --- LÓGICA DE APROBACIÓN BASADA EN ROL ---
        $user_role = $this->session->userdata('role');
        if ( $user_role == 'is_admin' ) {
            // Si es Admin o Subadmin, se aprueba automáticamente
            $productos_data['approved'] = 1;
            $productos_data['time_approved'] = date("Y-m-d H:i:s");
        } else {
            // Para cualquier otro rol, queda pendiente de aprobación
            $productos_data['approved'] = 0;
            $productos_data['time_approved'] = null; // Sin fecha de aprobación
        }
